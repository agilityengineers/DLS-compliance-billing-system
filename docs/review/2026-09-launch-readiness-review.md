...
| # | Sev | Finding | Where | Consequence |
|---|---|---|---|---|
| 15 | C | **Queued writes dropped after ~1 minute of transient failures.** Backoff is constant 5–10 s; after 8 attempts the queue item is deleted; the draft was already deleted at submit. The note stays on the device flagged `synced: 0`, but nothing re-queues it and the rejection list is in-memory only. | `lib/offline/syncEngine.ts:29-30,116-121,135`, `components/field/note-form.tsx:152`, `lib/offline/db.ts:146-169` | A short server outage strands signed progress notes on phones. Fixed in PR-D (backoff with cap, durable parked list, draft kept until ack). |
| 16 | H | **Any 401 wipes the device, including unsynced work.** | `lib/offline/syncEngine.ts:95-99`, `lib/offline/wipe.ts` | A revoked/expired refresh token destroys queued notes. Fixed in PR-D (401 pauses sync and asks for sign-in). |
...
| 19 | H | `/api/sync` validates only `payload.id`; `staff_id`, `client_id`, `administered_by`, visit status flips and timesheet targets are trusted. RLS is the only backstop in real mode; notes RLS never constrains `client_id`; demo mode has none. | `app/api/sync/route.ts:23-28,94-96,114-117,125`, `supabase/policies/05_progress_notes.sql:9-15` | A field user can write a billable note for any client. Fixed in PR-D (per-table payload schemas, session-bound identity, notes policy re-asserts the visit's client). |
