...
| 0.6 Real-database breakers | Fix the timesheet append that fails silently, the billing-cap check that stops working after 300 notes, the medication "missed" record, and the geofence trigger that made every GPS clock-in fail against the real database; add an in-process Postgres test harness so these rules are verified on every test run | Silent failures that would surface at launch as missing hours, no clock-ins, or over-authorization claims | — | 3 days |
