...
import { getSessionContext, requireRealAdmin } from "@/lib/auth/session";
...
export async function startImpersonation(targetUserId: string): Promise<{ ok: boolean; error?: string }> {
  ...
  // Read-only impersonation must leave a trace too (HIPAA §164.312(b)):
  // record the start under the admin's real identity, before any mutation.
  const audited = await recordImpersonationEvent("impersonation_started", ctx.realUser!.id, targetUserId, target.full_name as string);
  if (!audited.ok) {
...
/**
 * Audit rows for impersonation start/stop. audit_trails has no INSERT policy
 * (only the trigger writes it), so this is a documented service-role site;
 * performed_by and impersonating are set explicitly.
 */
...
export async function stopImpersonation(): Promise<{ ok: boolean }> {
  // No role check needed — exiting impersonation is always safe. Record the
  // stop (best effort) BEFORE the cookies go, while the target is still known.
  const ctx = await getSessionContext();
  if (ctx.impersonating && ctx.realUser && ctx.effectiveUser) {
    if (isDemoMode()) {
      getDemoStore().audit("impersonation", "INSERT", ctx.effectiveUser.id, null,
        { event: "impersonation_stopped", target: ctx.effectiveUser.full_name }, ctx.auditCtx);
    } else {
      await recordImpersonationEvent("impersonation_stopped", ctx.realUser.id, ctx.effectiveUser.id, ctx.effectiveUser.full_name);
    }
  }
  const jar = cookies();
