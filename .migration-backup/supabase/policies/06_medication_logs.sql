...
-- write only rows they administer. WITH CHECK re-asserts the client
-- assignment (previously an update could repoint client_id to an
-- unassigned client). A 'Missed' record has no administrator, so the
-- administered_by check applies only to Administered/Refused (see 0006).
